...
import { buildOrbitalDashboardModel, safeProgress } from './orbital-dashboard-model';
...
describe('orbital dashboard model', () => {
  ...
  it('builds time-aware and data-aware dashboard copy', () => {
    const model = buildOrbitalDashboardModel(input, 19);
    expect(model.greeting).toBe('Good evening');
    expect(model.calories.percent).toBe(42);
    expect(model.insights[0].body).toContain('water');
    expect(model.insights[1].body).toContain('fibre');
  });
