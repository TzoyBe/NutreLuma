...
export function DailyPlanPanel({ date }: { date: string }) {
  ...
  async function generate(force = false) {
    setLoading(true);
    try {
      const result = await api.post<{ plan: Plan }>('/api/meal-plan', { date, force });
      setPlan(result.plan);
    } catch (error) {
      toast.push(error instanceof ApiClientError ? error.message : t('recipes.generateFailed'), 'error');
    } finally {
  ...
  return (
    <section className="space-y-4">
        ...
        <>
          <Card>
            <CardContent className="grid gap-3 p-4 sm:grid-cols-5">
              {[
                [t('recipes.calories'), plan.remainingTarget.calories],
                [t('recipes.protein'), `${plan.remainingTarget.proteinGrams}g`],
                [t('recipes.carbs'), `${plan.remainingTarget.carbohydrateGrams}g`],
                [t('recipes.fat'), `${plan.remainingTarget.fatGrams}g`],
                [t('recipes.fibre'), `${plan.remainingTarget.fiberGrams}g`],
              ].map(([label, value]) => (
                <div key={String(label)}>
                  <p className="text-xs text-muted-foreground">
                    {t('recipes.remaining')} {label}
                  </p>
      ...
      <p className="text-xs text-muted-foreground">{t('recipes.disclaimer')}</p>
    </section>
  );
}
    ...
    <Card className="flex h-full flex-col">
      ...
      <CardContent className="flex flex-1 flex-col gap-3">
        <div className="flex flex-wrap gap-2 text-sm">
          <span className="font-semibold">{Math.round(recipe.estimatedCalories)} kcal</span>
          <span className="flex items-center gap-1 text-muted-foreground">
            <Clock3 className="h-4 w-4" aria-hidden="true" />
            {recipe.preparationTimeMinutes} min - {recipe.difficulty}
          </span>
        </div>
        <p className="text-xs text-muted-foreground">
          P {recipe.macros.proteinGrams}g - C {recipe.macros.carbohydrateGrams}g - F{' '}
          {recipe.macros.fatGrams}g - {t('recipes.fibre')} {recipe.macros.fiberGrams}g
        </p>
        <details>
          <summary className="cursor-pointer text-sm font-medium">{t('recipes.ingredientsSteps')}</summary>
        ...
        {recipe.allergenWarnings.length ? (
          <p className="rounded-md border border-amber-500/30 bg-amber-500/10 p-2 text-xs text-amber-200">
            {t('recipes.allergens')}: {recipe.allergenWarnings.join(', ')}
          </p>
        ) : null}
